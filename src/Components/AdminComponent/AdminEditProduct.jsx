...
import { adminProductUpdationSubmitAPI } from '../../axios/allAPI/adminAPI';
...
function  AdminEditProduct(product) {
    ...
    const [existingProductValue,setExistingProductValue]=useState({
    ...
    const adminProductUpdationSubmit=async()=>{
        // console.log("testinggg type of product.product.uploadimages",product.product.uploadimages )
        const token =sessionStorage.getItem("token")
        const {productid,title,description,category,price,uploadimages}=existingProductValue
...
>
  <Box sx={style}>
    ...
    <div className='h-screen bg-no-repeat bg-center bg-cover w-full bg-[url("https://c0.wallpaperflare.com/preview/394/192/959/black-apple-watch-on-iphone-x.jpg")]'>
      <div className='w-full  h-28  text-slate-400 font-Lobster text-5xl flex  justify-center ' >Edit Products</div>
      ...
      <input type='text' value={existingProductValue.title}  onChange={(e)=>setExistingProductValue({...existingProductValue,title:e.target.value})} className='bg-white focus:outline-none w-[70%] h-full rounded-lg text-center text-lg font-fourthStyle opacity-60 focus:opacity-90' placeholder='Product Title'></input>
      </div>
      ...
      <textarea type="text" value={existingProductValue.description} onChange={(e)=>setExistingProductValue({...existingProductValue,description:e.target.value})} className='bg-white focus:outline-none w-[70%] text-center rounded-lg  text-lg font-fourthStyle opacity-60 focus:opacity-90'  placeholder='Description'></textarea>
      </div>
      ...
      <input type='text' value={existingProductValue.category} onChange={(e)=>setExistingProductValue({...existingProductValue,category:e.target.value})} className='bg-white focus:outline-none w-[70%] h-full rounded-lg text-center text-lg font-fourthStyle opacity-60 focus:opacity-90' placeholder='Category'></input>
      </div>

      <div className='w-[70%] h-[8%] flex gap-9 lg:gap-6 flex-shrink items-center'>
      <label className='text-white font-fourthStyle text-xl lg:text-3xl w-[35%] lg:w-[30%]'>Price</label>
      <input type='text' value={existingProductValue.price} onChange={(e)=>setExistingProductValue({...existingProductValue,price:e.target.value})} className='bg-white focus:outline-none w-[70%] h-full rounded-lg text-center text-lg font-fourthStyle opacity-60 focus:opacity-90' placeholder='Price'></input>
      </div>

      <div className='w-[70%] h-[8%] flex gap-6 flex-shrink items-center'>
      <label className='text-white font-fourthStyle text-xl lg:text-3xl w-[35%] lg:w-[30%]'>Upload Image</label>

     {displayFileInputForUpdation? <input  type='file' onChange={(e)=>setExistingProductValue({...existingProductValue,uploadimages:e.target.files})} className='bg-white  focus:outline-none w-[70%] h-full rounded-lg text-center text-lg font-fourthStyle opacity-60 focus:opacity-90' multiple></input>
     ...
     <div className='flex gap-3 justify-center mt-7'>
      ...
      <button onClick={adminProductUpdationSubmit}  className='h-[50px] rounded-md w-[20%] bg-fuchsia-200 hover:bg-fuchsia-300'>Submit</button>
